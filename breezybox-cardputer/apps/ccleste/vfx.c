...
static DeathParticle *death_particles;
...
static DashParticle *dash_particles;
...
void vfx_init(void) {
    snow           = calloc(SNOW_COUNT,           sizeof(SnowParticle));
    clouds         = calloc(CLOUD_COUNT,          sizeof(Cloud));
    smoke          = calloc(SMOKE_COUNT,          sizeof(SmokeParticle));
    death_particles = calloc(DEATH_PARTICLE_COUNT, sizeof(DeathParticle));
    dash_particles  = calloc(DASH_PARTICLE_COUNT,  sizeof(DashParticle));
    lifeups        = calloc(LIFEUP_COUNT,         sizeof(LifeupText));
    memset(&orb, 0, sizeof(orb));

    if (!snow || !clouds || !smoke || !death_particles || !dash_particles || !lifeups) {
        return; /* OOM — game will draw nothing, which is safe */
    }
    ...
    /* smoke/death/dash/lifeup arrays are zero-initialized by calloc (active=false) */
}

void vfx_cleanup(void) {
    free(snow);           snow = NULL;
    free(clouds);         clouds = NULL;
    free(smoke);          smoke = NULL;
    free(death_particles); death_particles = NULL;
    free(dash_particles);  dash_particles = NULL;
    free(lifeups);        lifeups = NULL;
}

void vfx_draw_bg(void) {
    if (!clouds_enabled || !clouds) return;
...
void vfx_update(void) {
    ...
    if (!smoke || !death_particles || !dash_particles || !lifeups) return;
