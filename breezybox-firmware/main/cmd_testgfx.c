...
int cmd_testgfx(int argc, char **argv)
{
    ...
    int w = rgb_display_get_fb_width();
    int h = rgb_display_get_fb_height();
    if (w <= 0 || h <= 0) {
        rgb_display_set_mode(SM_TEXT);
        printf("Graphics mode did not expose a framebuffer.\n");
        return 1;
    }
    ...
    while (frame_count < max_frames) {
        ...
        for (int i = 0; i < RECT_PER_FRAME; i++) {
            ...
            rgb_gfx_rectfill(x + (bw / 4), y + (bh / 4), bw / 4, bh / 4, (uint8_t)((color + 48) & 0xFF));
            if (color == 0) color = 1;
            x += vx; y += vy;
            if (x <= 0 || x + bw >= w) {
                vx = -vx;
