...
static int s_cursor_sync_enabled = 1;
...
static void sync_display_cursor(void)
{
    int col, row, visible;
...
void my_console_scrollback(int delta_lines)
{
    vterm_scrollback(delta_lines);
    sync_display_cursor();
}

void my_console_scrollback_bottom(void)
{
    vterm_scrollback_bottom();
    sync_display_cursor();
}

int my_console_scrollback_active(void)
{
    return vterm_scrollback_active() ? 1 : 0;
}

void my_console_set_cursor_sync_enabled(int enabled)
{
    s_cursor_sync_enabled = enabled ? 1 : 0;
    if (s_cursor_sync_enabled) {
        sync_display_cursor();
    }
}
