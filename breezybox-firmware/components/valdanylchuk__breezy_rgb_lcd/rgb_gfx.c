...
static void draw_small_char(int x, int y, char ch, uint8_t fg_color, int bg_color)
{
    int w, h;
    uint8_t *fb = get_fb(&w, &h);
    if (!fb) return;
...
void rgb_gfx_text_small(int x, int y, const char *text, uint8_t fg_color, int bg_color)
{
    ...
    while (*text) {
        if (*text == '\n') {
            ...
            draw_small_char(cx, cy, *text, fg_color, bg_color);
            cx += 6;
        }
        ++text;
    }
...
static void draw_term16_char(int x, int y, char ch, uint8_t fg_color, int bg_color)
{
    int w, h;
    uint8_t *fb = get_fb(&w, &h);
    if (!fb) return;
    ...
    const uint8_t *glyph = &terminus16_glyph_bitmap[(c - 0x20) * 16];
...
void rgb_gfx_text_term16(int x, int y, const char *text, uint8_t fg_color, int bg_color)
{
    ...
    while (*text) {
        if (*text == '\n') {
            ...
            draw_term16_char(cx, cy, *text, fg_color, bg_color);
            cx += 8;
        }
        ++text;
    }
